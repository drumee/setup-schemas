  ...
  */
  async importTutorial() {
    ...
    for (let t of tutorials) {
      ...
      let filename = basename(t.src.replace(re, ''));
      let target = `https://${vhost}${node.ownpath}`;
      let sql = `REPLACE INTO tutorial SELECT ?, ?`;
      console.log(`UPDATING TUTORIAL`, { sql, filename, target })
      await this.yp.await_query(sql, filename, target);
    }
  }
  ...
  async importFile(url, dest) {
    ...
    this.debug(`Importing from ${url} -> ${pathname}`);
    if (id != null) {
  ...
  async importContent(vhost) {
    ...
    for (let node of nodes) {
      if (!/^(hub|folder)$/i.test(node.filetype)) {
        ...
        await this.importFile(url, dest);
      }
    }
  }
}
