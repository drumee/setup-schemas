...
function ensure_app_user(json) {
  ...
  let cmd;
  if (json.password) {
    cmd = `mysql -u${user} -p${json.password} -e "SELECT 'user ${user} uses password'"`;
  } else {
    cmd = `mysql -u${user} -e "SELECT user ${user} uses socket"`;
  }
  let r = exec(cmd);
  if (r.code === 0) {
    return true;
...
function user_exists(json) {
  let cmd = `mysql -e "SELECT user FROM mysql.user WHERE user='${json.user}' AND host='${json.host}'"`;
  let r = exec(cmd);
...
function grant_privilege(json, db = '*') {
  ...
  let cmd;
  if (db == '*') {
    cmd = `mysql -e "GRANT ${GRANTS} ON *.* TO ${user}"`;
  } else if (db) {
    cmd = `mysql -e "GRANT ALL PRIVILEGES ON ${db}.* TO ${user}"`;
  }
  r = exec(cmd);
  if (r.code !== 0) {
...
function reset_user(json) {
  let user = `'${json.user}'@'${json.host}'`;
  let cmd = `mysql -e "DROP USER IF EXISTS ${user}"`;
...
function create_user(json) {
  ...
  let r;
  if (json.password) {
    cmd = `mysql -e "CREATE OR REPLACE USER ${user} IDENTIFIED BY '${json.password}'"`;
  } else {
    cmd = `mysql -e "CREATE OR REPLACE USER ${user} IDENTIFIED VIA unix_socket"`;
  }
  r = exec(cmd);
  if (r.code === 0) {
...
function update_user(json) {
  ...
  if (isEmpty(json.password) || ['localhost', '127.0.0.1'].includes(json.host)) {
    ...
    cmd = `mysql -e "ALTER USER IF EXISTS ${user} IDENTIFIED BY '${json.password}'"`;
  }
...
function get_tmpdb() {
  ...
  let cmd = `mysql -e "show databases like '${name}'"`;
  let o = exec(cmd);
  let i = 0;
  if (o.code !== 0) {
  ...
  while (!isEmpty(o.stdout) && i < 100) {
    ...
    cmd = `mysql -e "show databases like '${name}'"`;
    o = exec(cmd);
    i++;
  }
...
function drop_tmpdb(name) {
  let cmd = `mysql -e "drop database if exists ${name}"`;
  let o = exec(cmd);
}
