  ...
  */
  ...
  async updateEntries(opt) {
    ...
    if (type == 'drumate') {
      ...
      if (category == "regular") {
        let sql = "SELECT e.db_name FROM hub h INNER JOIN entity e USING(id) WHERE h.owner_id=? AND h.serial=0";
        let { db_name } = this.yp.await_query(sql, newId);
        sql.push(`UPDATE ${db_name}.permission SET entity_id=? WHERE entity_id=?`);
      }
    } else {
