const {
  Attr, uniqueId, toArray, sysEnv, Cache, Constants
...
class Drumate extends Common {
  ...
  */
  ...
  async updateEntries(opt) {
    ...
    let s = `UPDATE entity SET home_dir=?, home_id=?, db_name=? WHERE id=?`;
    await this.yp.await_query(s, home_dir, home_id, db_name, newId);
