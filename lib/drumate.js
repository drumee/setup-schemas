  ...
  async updateEntries(opt) {
    ...
    if (type == 'drumate') {
      ...
      if (category == "regular") {
        ...
        this.debug("Updating wicket 1/2", s, newId);
        let r = this.yp.await_query(s, newId) || [];
        let { db_name } = r[0] || r;
        this.debug("WICKET DB = ", db_name);
        if(db_name){
          s = `UPDATE ${db_name}.permission SET entity_id=? WHERE permission=63`;
          this.debug("Updating wicket 2/2", s, newId);
          this.yp.await_query(s, newId);  
        }
      }
    } else {
